...
/** Документ в блоке «Стандартный набор» брифа — фиксированный состав (любая галочка матрицы НСИ). */
export function isStandardBlockDoc(doc: StandardDocument): boolean {
  return hasStandardMatrixEntry(doc);
}

/** Документ в блоке «Дополнительные» брифа — фиксированный состав (can_extra в НСИ). */
export function isAdditionalBlockDoc(doc: StandardDocument): boolean {
  return isAdditionalCatalogDoc(doc);
}

/** Включён в стандартный набор для данного типа (дефолт Да/Нет, не состав блока). */
...
export function resolveDocumentRow(
  ...
  const baseDefault = existing?.extra ? false : stdDefault;
  return {
...
export function mergeStandardDocumentsIntoCriteria(
  ...
  return { ...criteria, standard_documents: state, groups } as import('./sellerCriteria').SellerCriteria;
}
