...
import { EXPORT_VERSION } from './briefingExportTypes';
...
function buildExportPayload(briefingId: number, blocks: ExportBlocks) {
  ...
  if (blocks.customer) {
    ...
    payload.customer = {
      ...
      industry_id: full.industry_id as number | null,
  ...
  if (blocks.fs) {
    ...
    const mappedItems = fsItems.map(item => {
        ...
        inactive_for_customer: Boolean(item.inactive_for_customer),
  ...
  if (blocks.problems) {
    const allProblems = db.prepare(`SELECT id, name FROM problems WHERE parent_id IS NULL ORDER BY name`).all() as {
  ...
  if (blocks.widgets) {
    const widgetRows = db.prepare(`
      SELECT bws.solution_id, bws.widget_id, w.name, w.description, w.image_path, sol.name as solution_name
    ...
    const allWidgets = db.prepare(`
      SELECT w.id, w.name, w.description, w.image_path, swm.solution_id, sol.name as solution_name
      FROM widgets w
...
export function buildBriefingHtmlExport(briefingId: number, blocks: ExportBlocks): string | null {
  const payload = buildExportPayload(briefingId, blocks);
